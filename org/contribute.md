...
Read the [](pyomo-style-guide.md) first and work down its checklist. It covers imports, model-building
functions, constraint decorators, checking the solver status, seeding random number generators, the Colab
setup cell and the solution markers.

Then follow these tips to make sure your project notebook looks great:
...
* If you are creating class excerises, put the solutions (in a code cell) between the comments `### BEGIN SOLUTION` and `### END SOLUTION`. When I publish the notebooks to the website, a script will automatically remove the answer and replace it with `# Add your solution here`. See [](pyomo-style-guide.md) for how the markers must be formatted.
