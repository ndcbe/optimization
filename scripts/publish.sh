...
To publish:
  ...
  3. Confirm no solutions leaked:
     ...
     It must print "clean" and exit 0. This replaced a pair of hand-run checks,
     one of which (`grep -rl "BEGIN SOLUTION"`) is worthless on its own: a
     solution leaks through a cell's stored OUTPUTS as readily as through its
     source, and the grep sees only the source. It was clean throughout the
     2026-08-21 leak. ./scripts/build_local.sh runs the checker for you.

     To see it fail on purpose:  python ./scripts/check_solution_leaks.py --selftest
